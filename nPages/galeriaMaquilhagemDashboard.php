...
<div class="search-div">
	...
	<input class="search-bar" type="text" placeholder="Pesquise aqui..." title="Pesquise aqui...">
</div>

<!-- TABLE MAQUILHAGEM -->
<table id="table-maquilhagem" class="table table-galeria">
	<caption>Imagens galeria maquilhagem</caption>
	<thead>
		<tr>
	...
	<tbody id="tbody">
		...
		# COUNT ALL ROWS
		$sqlNumRegistos = $conn->query("SELECT COUNT(*) FROM fotogaleria WHERE id_servico = 2;");
		$totalRows = $sqlNumRegistos->fetch_array()[0];
		$totalPages = ceil($totalRows / $rowsPerPage);


		# QUERY
		$users = $conn->query("SELECT id_fotografia, descricao, data_publicacao, caminho, id_servico FROM fotogaleria WHERE id_servico = 2 ORDER BY id_fotografia DESC LIMIT $offset, $rowsPerPage;");
...
<nav class="table-pagination" aria-label="Navegação de páginas">
	<ul class="pagination justify-content-end">
		<li class="page-item <?php if($pageNum <= 1){ echo 'disabled'; } ?>">
			<a href="<?php if($pageNum <= 1){ echo '#'; } else { echo "?pagina=nPages/galeriaDashboard.php&type=maquilhagem&page=".($pageNum - 1); } ?>" class="page-link">Anterior</a>
		</li>
		<?php 
		...
		while ($i <= $totalPages) {
			...
			echo "<li class='page-item "; if ($i == $_GET["page"]) echo 'active'; echo "'><a class='page-link' href='?pagina=nPages/galeriaDashboard.php&type=maquilhagem&page=$i'>$i</a></li>";
			$i++;
		}

		?>
		<li class="page-item <?php if($pageNum >= $totalPages){ echo 'disabled'; } ?>">
			<a href="<?php if($pageNum >= $totalPages){ echo '#'; } else { echo "?pagina=nPages/galeriaDashboard.php&type=maquilhagem&page=".($pageNum + 1); } ?>" class="page-link">Próxima</a>
		</li>   
	</ul>
</nav>
